...
const insertNoteQuery = async (note) => {
  ...
  let connection;
  try {
    const { title, text, image, category, user_id, uuid } = note;
    ...
    const [newNote] = await connection.query(
      `INSERT INTO notes  (title, text, image, category, user_id, uuid, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [title, text, image, category, user_id, uuid, new Date()]
    );
